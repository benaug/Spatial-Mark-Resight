...
###Simulating data with "very generous" settings here because of difficulty estimating
#overdispersion with imperfect individual ID. 20-22 marked individuals per session with
#telemetry. Estimates roughly unbiased with approx nominal coverage with these settings.
#I am unsure how "good" the data needs to be to get decent estimates with negbin obsmod.
...
source("NimbleModel SMR Multisession Premarked NegBin.R")
source("NimbleFunctions SMR Multisession Premarked NegBin.R")
...
data=sim.SMR.multisession(N.session=N.session,lambda=lambda,theta.d=theta.d,n.marked=n.marked,marktype=marktype,
...
#setting to truth here, don't do this in practice

#This function structures the simulated data to fit the model in Nimble (some more restructing below)
#Also checks some inits
nimbuild=init.SMR.multisession(data,inits,M1=M1,M2=M2,marktype=marktype,obstype="negbin")
...
Niminits <- list(N=N.init,N.UM=N.UM.init,
                 ...
                 theta.unmarked=theta.unmarked.init,lam0.fixed=0.75,sigma.fixed=0.75, #one init per lam0.fixed, sigma.fixed, theta.d.fixed
                 theta.d.fixed=0.2,D=0.25)
